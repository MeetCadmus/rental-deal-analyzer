...
export interface DealStore { deals: Deal[]; activeId: string | null; deleted?: Record<string, number> }
...
export const setTomb = (t: Record<string, number>): void => { TOMB = t || {}; };

export function persistDeals(deals: Deal[], activeId: string | null): void {
  try { localStorage.setItem(DEALS_KEY, JSON.stringify({ deals, activeId, deleted: TOMB })); } catch { /* ignore */ }
}
...
export function mergeDealStores(localStore?: Partial<DealStore>, cloudStore?: Partial<DealStore>): DealStore {
  ...
  const addTomb = (t?: Record<string, number>) => { if (t) for (const id in t) { if (!(id in tomb) || t[id] > tomb[id]) tomb[id] = t[id]; } };
  addTomb(localStore.deleted); addTomb(cloudStore.deleted);
  const byId = new Map<string, Deal>();
  const add = (d: Deal) => { if (!d || !d._id) return; const ex = byId.get(d._id); if (!ex || (d._ts || 0) > (ex._ts || 0)) byId.set(d._id, d); };
  ...
  for (const id in tomb) { if (!survive.has(id)) deleted[id] = tomb[id]; }
  const has = (id: string | null | undefined) => !!id && survive.has(id);
  const activeId = has(localStore.activeId) ? localStore.activeId!
    : has(cloudStore.activeId) ? cloudStore.activeId!
      : (deals.length ? deals[deals.length - 1]._id! : null);
...
export function loadDealStore(): { deals: Deal[]; activeId: string } {
  ...
  const deals: Deal[] = [];
  try { const sc = JSON.parse(localStorage.getItem("re_scenarios") || "[]"); if (Array.isArray(sc)) sc.forEach((s) => deals.push(makeDeal(s, { label: s._name, ts: s._ts, created: s._ts }))); } catch { /* ignore */ }
  try { const a = JSON.parse(localStorage.getItem("re_autosave") || "null"); if (a && typeof a === "object" && Object.keys(a).length) deals.push(makeDeal(a, {})); } catch { /* ignore */ }
