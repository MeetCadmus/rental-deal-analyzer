...
export function buildAIPrompt(s: Partial<Deal> | undefined, listing?: string): string {
  ...
  return [
    ...
    '    "floodZone": string,                             // FEMA flood zone + plain risk, e.g. "Zone X — minimal" / "Zone AE — high, 100-yr floodplain"',
    '    "elevation": string,                             // approx elevation above sea level + coastal/storm-surge exposure, e.g. "~1,050 ft, inland — no surge"',
    '    "storms": string,                                // hurricane / tornado / severe-wind exposure for the region',
    '    "wildfire": string,                              // wildfire / wildland-urban-interface risk, e.g. "low — urban" / "high — WUI"',
    '    "heat": string,                                  // extreme-heat & drought trend',
