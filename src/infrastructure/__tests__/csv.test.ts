...
test("CSV: full deal state round-trips exactly (nested, arrays, types, commas)", () => {
  const state = {
    address: "123 Maple, Atlanta, GA",
    notes: "rents low, new roof\nmotivated seller",
    price: 620000,
    units: [
      { id: 1, label: "Unit 1", rent: 1550, beds: 2 },
      { id: 2, label: "Unit 2", rent: 1150, beds: 1 },
    ],
...
test("CSV: types are restored (numbers, booleans, empty arrays) not left as strings", () => {
  const back = M.csvToState(
    M.stateToCSV({
      price: 500000,
      financing: { rate: 7.25 },
      repairs: { include: true },
      comparables: [],
      notes: "",
    }),
  );
