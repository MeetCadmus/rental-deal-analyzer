...
export const setSupa = (c: SupabaseClient | null) => { supa = c; };
...
export const setTouched = (v: boolean) => { touched = v; };
...
export interface WorkspaceStore {
  deals: Deal[]; activeId: string; state: Deal;
  undo: { deal: Deal; idx: number } | null;
  dealsOpen: boolean; showEx: boolean; showUD: boolean; selEx: string | null;
  isPrinting: boolean; toast: string;
  user: User | null; sync: SyncState;
  // cloud
  setUser: (u: User | null) => void; setSync: (s: SyncState) => void;
  applyStore: (st: Partial<DealStore>) => void;
  signIn: () => void; signOut: () => void;
  // ui setters
  setDealsOpen: (v: boolean) => void; setShowEx: (v: boolean) => void; setShowUD: (v: boolean) => void;
  setToast: (t: string) => void; setIsPrinting: (v: boolean) => void; setSelEx: (v: string | null) => void;
  ...
  addUnit: () => void; remUnit: (i: number) => void; setInsights: (v: unknown) => void;
  applyListing: (pl: ParsedListing) => void; applyAI: (o: Record<string, any>) => void;
  // deal actions
  addDeal: (data: Partial<Deal>, label?: string) => string;
  switchDeal: (id: string) => void; newDeal: () => void; duplicateDeal: (id: string) => void;
  renameDeal: (id: string, label: string) => void; deleteDeal: (id: string) => void; undoDelete: () => void;
  loadEx: (ex: Example) => void;
  exportCSV: () => void; importCSV: () => void; copyShareLink: () => void;
  exportAllDeals: () => void; importAllDeals: () => void; importShared: () => void;
  handlePrint: () => void;
}

export const useWorkspace = create<WorkspaceStore>((set, get) => ({
  deals: boot.deals, activeId: boot.activeId,
  state: fullState(boot.deals.find((d) => d._id === boot.activeId) || boot.deals[0]),
  undo: null, dealsOpen: false, showEx: false, showUD: false, selEx: null, isPrinting: false, toast: "",
  user: null, sync: "idle",
  ...
  applyStore: (st) => {
    ...
    setTomb(st.deleted || {}); touched = false;
  ...
  signIn: () => { if (supa) signInWithGoogle(supa); },
  signOut: () => { if (supa) sbSignOut(supa); set({ user: null, sync: "idle" }); },
  ...
  setUnit: (i, k, v) => set((s) => { const u = [...s.state.units]; u[i] = { ...u[i], [k]: v }; return { state: { ...s.state, units: u } }; }),
  addUnit: () => set((s) => { const next = s.state.units.reduce((m, u) => { const mm = /(\d+)\s*$/.exec(u.label || ""); return Math.max(m, mm ? parseInt(mm[1], 10) : 0); }, 0) + 1; return { state: { ...s.state, units: [...s.state.units, { id: uid(), label: "Unit " + next, rent: 1400, beds: 1, bath: 1, sqft: 700 }] } }; }),
  remUnit: (i) => set((s) => ({ state: { ...s.state, units: s.state.units.filter((_, j) => j !== i) } })),
  setInsights: (v) => set((s) => ({ state: { ...s.state, insights: v } })),

  applyListing: (pl) => set((s) => {
    const p = s.state; const np: Deal = { ...p };
  ...
  addDeal: (data, label) => {
    touched = false; const d = makeDeal(data, label ? { label } : {});
    const n = [...get().deals, d]; persistDeals(n, d._id!);
    set({ deals: n, activeId: d._id!, state: fullState(d) });
    return d._id!;
  },
  switchDeal: (id) => { if (id === get().activeId) return; const d = get().deals.find((x) => x._id === id); if (!d) return; touched = false; set({ activeId: id, state: fullState(d), selEx: null }); },
  newDeal: () => { get().addDeal(BLANK); set({ selEx: null, dealsOpen: false }); },
  duplicateDeal: (id) => { const src = get().deals.find((x) => x._id === id); if (src) get().addDeal(src, dealTitle(src) + " (copy)"); },
  renameDeal: (id, label) => { const n = get().deals.map((d) => d._id === id ? { ...d, _label: label } : d); persistDeals(n, get().activeId); set({ deals: n }); },
  deleteDeal: (id) => {
    const deals = get().deals; const idx = deals.findIndex((d) => d._id === id); if (idx < 0) return;
    const removed = deals[idx]; setTomb({ ...getTomb(), [id]: Date.now() });
    let n = deals.filter((d) => d._id !== id); let act = get().activeId;
    const patch: Partial<WorkspaceStore> = {};
    if (id === get().activeId) { if (!n.length) { n = [makeDeal(INIT, {})]; } act = (n[Math.max(0, idx - 1)] || n[n.length - 1])._id!; touched = false; patch.activeId = act; patch.state = fullState(n.find((d) => d._id === act)); patch.selEx = null; }
    persistDeals(n, act); set({ deals: n, undo: { deal: removed, idx }, ...patch });
    if (undoTimer) clearTimeout(undoTimer);
    undoTimer = setTimeout(() => { if (get().undo?.deal === removed) set({ undo: null }); }, 7000);
  },
  undoDelete: () => {
    const undo = get().undo; if (!undo) return; const d = undo.deal;
    { const t = { ...getTomb() }; delete t[d._id!]; setTomb(t); }
    const n = [...get().deals]; n.splice(Math.min(undo.idx, n.length), 0, d); persistDeals(n, d._id!);
    touched = false; set({ deals: n, activeId: d._id!, state: fullState(d), undo: null });
    if (undoTimer) clearTimeout(undoTimer);
  },
  loadEx: (ex) => { touched = false; set({ state: fullState({ ...INIT, ...ex, closing: ex.closing || { ...DCC }, expenses: ex.expenses || { ...DEX }, projection: { ...INIT.projection, ...ex.projection } }), selEx: ex.id }); },

  exportCSV: () => { const S = get().state; const base = (S.address || "deal").replace(/[^a-z0-9]+/gi, "_").replace(/^_|_$/g, "").slice(0, 40) || "deal"; downloadFile(base + ".csv", stateToCSV(S as unknown as Record<string, unknown>)); },
  importCSV: () => {
  ...
  copyShareLink: () => {
    ...
    } catch { set({ toast: "Couldn't build a share link for this deal." }); }
  },
  importShared: () => {
    try {
  ...
  exportAllDeals: () => { const stamp = new Date().toISOString().slice(0, 10); downloadFile("re-deals-backup-" + stamp + ".json", JSON.stringify({ app: "re-investment-analyzer", version: 1, exportedAt: Date.now(), deals: get().deals, activeId: get().activeId }, null, 2), "application/json"); },
  importAllDeals: () => {
    ...
    inp.onchange = (e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (!f) return; const rd = new FileReader();
      rd.onload = () => { try {
        ...
        const n = [...get().deals, ...imported]; persistDeals(n, imported[0]._id);
        touched = false; set({ deals: n, activeId: imported[0]._id, state: fullState(imported[0]), selEx: null, toast: "Imported " + imported.length + " deal" + (imported.length > 1 ? "s" : "") });
      } catch (err) { set({ toast: "Couldn't import deals: " + (err as Error).message }); } };
      rd.readAsText(f); };
    inp.click();
  },
  handlePrint: () => {
    ...
    setTimeout(() => { window.print(); setTimeout(() => { set({ isPrinting: false }); try { document.documentElement.setAttribute("data-theme", prev || "light"); } catch { /* ignore */ } }, 300); }, 150);
  },
}));
