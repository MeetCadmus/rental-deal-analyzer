...
function ScenarioCompare({ deals, activeId, currentState }: { deals: Deal[]; activeId: string; currentState: Deal }) {
  ...
  const PSORT: Record<string, [string, (c: any) => any]> = { grade: ["Grade", c => c.score.pct], cf: ["Cash flow", c => c.R.cf], cap: ["Cap rate", c => c.R.capRate], irr: ["IRR", c => c.Y.irr], price: ["Price", c => c.st.price], recent: ["Recently edited", c => c.d._ts || 0], name: ["Name (A–Z)", c => c.d] };
  ...
  const computed = cols.map((c: any) => { const R = computeBase(c.st); const Y = computeYearly(c.st, R); return { ...c, R, Y, score: calcDealScore(R, Y) }; });
  const rows: any[] = [
    ...
    { l: "CF / unit / mo", f: (c: any) => fmtD(c.R.cf / c.R.numU / 12), col: (c: any) => c.R.cf >= 0 ? C.teal : C.red, v: (c: any) => c.R.cf / c.R.numU / 12, best: "max" },
  ...
  const SORTS: Record<string, [string, (c: any) => any]> = { irr: ["IRR", c => c.Y.irr], grade: ["Grade", c => c.score.pct], cf: ["Cash flow", c => c.R.cf], cap: ["Cap rate", c => c.R.capRate], coc: ["Cash-on-cash", c => c.R.coc], dscr: ["DSCR", c => c.R.dscr], totRet: ["Total return", c => c.Y.totRet] };
  ...
  const bestIdx = (row: any) => { if (!row.best || ordered.length < 2) return -1; let bi = 0; for (let i = 1; i < ordered.length; i++) { const better = row.best === "max" ? row.v(ordered[i]) > row.v(ordered[bi]) : row.v(ordered[i]) < row.v(ordered[bi]); if (better) bi = i; } return bi; };
  return <>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead><tr>
                ...
                {ordered.map((c: any, i: number) => <th key={i} style={{ padding: "7px 10px", textAlign: "right", borderBottom: "2px solid " + C.border, minWidth: 120 }}>
              ...
              <tbody>{rows.map((row, ri) => { const bi = bestIdx(row); return <tr key={ri} style={{ background: ri % 2 ? C.bg : C.white }}>
                ...
                {ordered.map((c: any, i: number) => { const col = row.lvl ? lvlCol[row.lvl(c)] : row.col ? row.col(c) : C.text; const win = i === bi; return <td key={i} style={{ padding: "6px 10px", textAlign: "right", fontWeight: win ? 800 : 600, color: col, fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap", background: win ? C.tealL : "transparent" }}>{win ? "★ " : ""}{row.f(c)}</td>; })}
              </tr>; })}</tbody>
            </table>
          </div>
