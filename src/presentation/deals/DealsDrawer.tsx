...
function DealsDrawer({ open, onClose, deals, activeId, liveTitle, onSelect, onNew, onRename, onDelete, onDuplicate, onExportAll, onImportAll }: { open: boolean; onClose: () => void; deals: Deal[]; activeId: string; liveTitle: string; onSelect: (id: string) => void; onNew: () => void; onRename: (id: string, label: string) => void; onDelete: (id: string) => void; onDuplicate: (id: string) => void; onExportAll: () => void; onImportAll: () => void }) {
  ...
  const SORTS: Record<string, [string, ((c: any) => any) | null]> = { recent: ["Recently edited", c => c.d._ts || 0], grade: ["Grade", c => c.sc.pct], cf: ["Cash flow", c => c.R.cf], cap: ["Cap rate", c => c.R.capRate], irr: ["IRR", c => c.Y.irr], price: ["Price", c => c.d.price], name: ["Name (A–Z)", null] };
  ...
  const startEdit = (d: Deal) => { setEditId(d._id ?? null); setEditVal(d._label || d.address || ""); };
  const commitEdit = () => { if (editId != null) onRename(editId, editVal.trim()); setEditId(null); };
  const xbtn = { fontSize: 10, padding: "3px 8px", borderRadius: 6, border: "1px solid " + C.border, background: C.bg, color: C.slate, cursor: "pointer", fontFamily: "inherit" } as const;
          ...
          return <div key={d._id} onClick={() => { if (!editing) { onSelect(d._id!); onClose(); } }} style={{ border: "1px solid " + (isA ? C.navy : C.border), background: isA ? C.hl : C.white, borderRadius: 10, padding: "9px 11px", marginBottom: 7, cursor: "pointer" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 9, color: C.muted, marginTop: 1 }}>✎ {fmtWhen(d._ts)} <span style={{ opacity: 0.6 }}>· {relTime(d._ts || 0)}</span>{d.aiSource ? <span> · {d.aiSource}</span> : ""}</div>
              </div>
