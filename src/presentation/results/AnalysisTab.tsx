...
export function AnalysisTab({ SEN, R, S, Y: _Y }: { SEN: Sensitivity; R: BaseMetrics; S: Deal; Y: YearlyResult }) {
  ...
  const loans = useMemo(
    () => calcLoanOptions(S.price, S.financing.downPct, S.financing.rate, S.financing.loanYears),
    [S.price, S.financing.downPct, S.financing.rate, S.financing.loanYears],
  );
        ...
        <div style={{ padding: "12px 13px", background: C.white }}>
          ...
          <div style={{ display: "flex", gap: 9, alignItems: "end", marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              <Field
                ...
                prefix="$"
                value={targetCF}
                onChange={setTargetCF}
                min={-5000}
                max={10000}
                step={50}
                sub="0 = break-even, positive = cash flow"
              />
            </div>
          </div>
          <div className={s.cols3} style={{ gap: 9 }}>
            {(
              [
                {
                  label: "Needed rent/unit",
                  val: fmtD(wnt.neededRentPU) + "/mo",
                  curr: fmtD(R.monRent / R.numU) + "/mo current",
                  delta: wnt.neededRentPU - R.monRent / R.numU,
                  good: wnt.neededRentPU <= R.monRent / R.numU,
                },
                {
                  label: "Max purchase price",
                  val: wnt.neededPrice ? fmtD(wnt.neededPrice) : "Not possible",
                  curr: fmtD(S.price) + " current",
                  delta: wnt.neededPrice ? wnt.neededPrice - S.price : null,
                  good: wnt.neededPrice != null && wnt.neededPrice >= S.price,
                },
                {
                  label: "Needed rate",
                  val: wnt.neededRate + "%",
                  curr: S.financing.rate + "% current",
                  delta: wnt.neededRate - S.financing.rate,
                  good: wnt.neededRate >= S.financing.rate,
                },
              ] as WntItem[]
            ).map((item) => (
              <div
                key={item.label}
                style={{ padding: "10px 11px", background: C.white, borderRadius: "calc(var(--c-rad) - 2px)", border: "1px solid " + C.border }}
              >
                ...
                {item.delta !== null && (
                  <div style={{ fontSize: 10, fontWeight: 600, color: item.good ? C.teal : C.red, marginTop: 2 }}>
                    {item.delta >= 0 ? "+" : ""}
                    {item.label.includes("Rate") ? item.delta.toFixed(2) + "%" : item.label.includes("rent") ? fmtD(item.delta) : fmtD(item.delta)} vs current
                  </div>
                )}
              </div>
            ))}
            ...
            <thead>
              <tr style={{ background: C.bg }}>
                ...
                {rVars.map((r) => (
                  ...
                  >
                    {r === 0 ? "Current" : (r > 0 ? "+" : "") + r.toFixed(1) + "%"}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {SEN.priceRate.map((row, ri) => (
                <tr key={ri}>
                  <td
                    style={{
                      padding: "5px 10px",
                      fontWeight: 600,
                      color: row.label === "Current" ? C.heading : C.text,
                      background: row.label === "Current" ? "var(--c-hl)" : C.white,
                      whiteSpace: "nowrap",
                    }}
                  >
                    {row.label === "Current" ? "Current" : row.label}{" "}
                    {row.label !== "Current" && <span style={{ fontSize: 9, color: C.slate }}>({fmtD(row.price)})</span>}
                  </td>
                  {row.cells.map((cell, ci) => {
                    const isCur = row.label === "Current" && cell.label === "Current";
                    return (
                      <td
                        key={ci}
                        style={{
                          padding: "5px 8px",
                          textAlign: "center",
                          background: cfBg(cell.cf),
                          fontWeight: isCur ? 800 : 500,
                          color: cfColor(cell.cf),
                          fontVariantNumeric: "tabular-nums",
                          outline: isCur ? "2px solid " + C.heading : "none",
                          outlineOffset: -2,
                        }}
                      >
                        {fmtD(cell.cf / 12)}/mo
          ...
          {SEN.rentCells.map((cell) => (
            <div
              ...
              style={{
                ...
                borderRadius: 8,
                textAlign: "center",
                border: "1px solid " + (cell.delta === 0 ? C.gold : "transparent"),
              }}
            >
              <div style={{ fontSize: 10, color: C.slate, marginBottom: 2 }}>
                {cell.delta === 0 ? "Current" : (cell.delta > 0 ? "+" : "") + fmtD(cell.delta) + "/unit"}
              </div>
          ...
          <table className={s.table} style={{ fontSize: 11 }}>
            ...
            <tbody>
              {loans.map((loan, i) => {
                ...
                const isCurrent = i === 0;
                return (
                  <tr key={loan.name} style={{ background: isCurrent ? "var(--c-hl)" : i % 2 === 0 ? C.white : C.bg }}>
                    <td
                      style={{
                        padding: "5px 8px",
                        fontWeight: isCurrent ? 600 : 400,
                        color: isCurrent ? C.heading : C.text,
                        textAlign: "right",
                        whiteSpace: "nowrap",
                      }}
                    >
                      {loan.name}
                      {isCurrent && (
                        <span style={{ fontSize: 9, marginLeft: 4, padding: "1px 5px", background: C.hl, color: C.heading, borderRadius: 4 }}>current</span>
                      )}
                    </td>
