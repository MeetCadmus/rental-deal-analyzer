...
interface ProjRow { l: string; v: string; f: string; c?: string; bold?: boolean }
interface ProjSection { t: string; rows: ProjRow[] }
...
export function ProjTrace({ R, Y, S }: { R: BaseMetrics; Y: YearlyResult; S: Deal }) {
  ...
  const sections: ProjSection[] = [
    ...
    { t: "Est. IRR = " + fmtP(Y.irr) + " — annualized, timing-aware return", rows: [
      ...
      { l: "Net sale proceeds", v: fmtD(netSale), f: "exit value " + fmtD(Y.exitVal) + " − " + sc + "% selling costs − loan payoff " + fmtD(last.balance || 0) },
    ] },
