  ...
  return <div>
    ...
    {!compact && <div style={{ borderRadius: "var(--c-rad)", padding: "4px 0 0", marginBottom: 11 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 }}>
        {([["Monthly CF", fmtD(R.cf / 12) + "/mo", R.cf >= 0], ["Per unit", fmtD(R.cf / R.numU / 12) + "/unit/mo", R.cf >= 0], ["Annual CF", fmtD(R.cf) + "/yr", R.cf >= 0], ["Cash needed", fmtD(R.cashIn), true]] as [string, string, boolean][]).map(([l2, val, pos]) => <div key={l2} style={{ background: C.white, border: "1px solid " + C.border, borderRadius: "calc(var(--c-rad) - 2px)", padding: "9px 11px" }}>
    ...
    <div style={{ padding: "9px 11px", background: C.white, border: "1px solid " + C.border, borderRadius: 10, marginBottom: 11 }}>
      ...
      {([["Down payment", fmtD(R.down)], ["Closing costs", fmtD(R.ccTotal)], ...(S.repairs.include && !S.repairs.unknown && S.repairs.amount > 0 ? [["Repairs", fmtD(S.repairs.amount)]] : [])] as [string, string][])
    ...
    {(() => {
      ...
      const cells: [string, string][] = [["Price / unit", fmtD(S.price / numU)], ["Price / sq ft", tsf ? fmtD(S.price / tsf) : "—"], ["Rent / sq ft", tsf ? ("$" + (R.monRent / tsf).toFixed(2) + "/mo") : "—"], ["GRM", fmtX(R.grm)]];
    ...
    <div style={{ border: "1px solid " + C.border, borderRadius: 11, overflow: "hidden" }}>
      ...
      <div style={{ padding: "6px 13px" }}>
        {[{ l: "1% rule (" + adjLbl + ")", v: fmtP(R.pct1), pill: pct1Pass ? "Passes" : R.pct1 >= R.adjThresh * 0.85 ? "Close" : "Below", lvl: (pct1Pass ? "good" : R.pct1 >= R.adjThresh * 0.85 ? "warn" : "bad") as "good" | "warn" | "bad", note: "Monthly rent ÷ price" }].map(m => <div key={m.l} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 0", gap: 6 }}>
