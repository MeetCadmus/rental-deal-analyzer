...
import type { Closing, CustomItem } from "../../domain/types";
...
export function ClosingCosts({ cc, setCC, price, loan, annTax, annIns, rate, collapsible, defaultOpen }: ClosingCostsProps) {
  ...
  const setI = (i: number, k: string, v: unknown) => setCC(p => { const a = [...p.customItems]; a[i] = { ...a[i], [k]: v } as CustomItem; return { ...p, customItems: a }; });
  return <Card title="Closing costs" icon="file" collapsible={collapsible} defaultOpen={defaultOpen} storeKey="closing" summary={collapsible ? fmtD(total) : undefined}>
    <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 12 }}>
      {([["quick", "Quick %"], ["detailed", "Itemized"]] as const).map(([id, lbl]) => { const on = cc.mode === id; return <button key={id} onClick={() => sf("mode", id)} style={{ padding: "5px 12px", borderRadius: 7, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 700, border: "1.5px solid " + (on ? C.navy : C.border), background: on ? C.navy : C.white, color: on ? "#fff" : C.slate }}>{lbl}</button>; })}
    ...
    {cc.mode === "detailed" && <div>
      ...
      <div style={{ background: "linear-gradient(90deg," + C.navy + "," + C.navyM + ")", borderRadius: 9, padding: "10px 14px", color: "#fff", marginTop: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 5 }}>
          {([["Lender", lT], ["Taxes", taxT], ["Title/atty", titleT], ["Prepaids", prepT]] as const).map(([l2, v2]) => <div key={l2} style={{ background: "rgba(255,255,255,0.08)", borderRadius: 6, padding: "4px 7px" }}><div style={{ fontSize: 9, opacity: 0.65 }}>{l2}</div><div style={{ fontSize: 11, fontWeight: 700, color: C.gold }}>{fmtD(v2)}</div></div>)}
        </div>
      </div>
    </div>}
  </Card>;
