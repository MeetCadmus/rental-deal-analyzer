...
import type { Expenses as ExpensesT, CustomExpense } from "../../domain/types";
...
export function Expenses({ ex, setEx, units, egi, price, collapsible, defaultOpen }: ExpensesProps) {
  ...
  const setCE = (i: number, k: string, v: unknown) => setEx(p => { const a = [...p.customExpenses]; a[i] = { ...a[i], [k]: v } as CustomExpense; return { ...p, customExpenses: a }; });
  return <Card title="Vacancy & Expenses" icon="percent" collapsible={collapsible} defaultOpen={defaultOpen} storeKey="expenses" summary={collapsible ? fmtD(totExp) + "/yr" : undefined}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 12 }}>
      {([["quick", "Quick %"], ["detailed", "Itemized"]] as const).map(([id, lbl]) => { const on = ex.mode === id; return <button key={id} onClick={() => sf("mode", id)} style={{ padding: "5px 12px", borderRadius: 7, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 700, border: "1.5px solid " + (on ? C.navy : C.border), background: on ? C.navy : C.white, color: on ? "#fff" : C.slate }}>{lbl}</button>; })}
      ...
      return <div>
        ...
        <div style={{ background: "linear-gradient(90deg," + C.navy + "," + C.navyM + ")", borderRadius: 9, padding: "10px 14px", color: "#fff", marginTop: 13 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 5 }}>
            {([["Tax & ins", taxIns], ["Mgmt", items.mgmt], ["Maint & reserves", maintRes], ["Other", other]] as const).map(([l2, v2]) => <div key={l2} style={{ background: "rgba(255,255,255,0.08)", borderRadius: 6, padding: "4px 7px" }}><div style={{ fontSize: 9, opacity: 0.65 }}>{l2}</div><div style={{ fontSize: 11, fontWeight: 700, color: C.gold }}>{fmtD(v2)}</div></div>)}
          </div>
        </div>
      </div>;
    })()}
