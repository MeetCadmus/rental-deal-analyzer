...
function QuickFill({ state, onListing, onAI, onSource }: { state: Deal; onListing: (pl: any) => void; onAI: (o: any) => void; onSource: (v: string) => void }) {
  ...
  const flash = (k: string) => { setMsg(null); setDone(k); setTimeout(() => setDone(d => d === k ? "" : d), 1600); };
  const ta: CSSProperties = { width: "100%", boxSizing: "border-box", padding: "7px 9px", fontSize: 12, border: "1px solid " + C.border, borderRadius: 8, fontFamily: "inherit", color: C.text, background: C.white, outline: "none", resize: "vertical", lineHeight: 1.45 };
  const btn: CSSProperties = { padding: "6px 12px", borderRadius: 8, background: C.navy, color: "#fff", border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 700 };
  ...
  const copyPrompt = () => {
    ...
    const merged: any = { ...state }; if (pl.address) merged.address = pl.address; if (num(pl.price) > 0) merged.price = num(pl.price);
