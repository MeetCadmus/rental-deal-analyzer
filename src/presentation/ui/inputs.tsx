...
export function MoneyInput({ value, onChange, label, sub, small, hint }: { value: number; onChange: (n: number) => void; label?: ReactNode; sub?: ReactNode; small?: boolean; hint?: ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", border: "1px solid " + C.border, borderRadius: 7, overflow: "hidden", background: C.white }}>
        ...
        <input ref={g.ref} type="text" inputMode="numeric" value={g.display} placeholder="0" onChange={g.onInput} onBlur={g.clearBuf}
...
interface FieldProps {
  label?: ReactNode; prefix?: ReactNode; suffix?: ReactNode; value: number; onChange: (n: number) => void;
  min?: number; max?: number; step?: number; sub?: ReactNode; disabled?: boolean; xs?: boolean;
  placeholder?: string; showZero?: boolean; tip?: string[];
}
export function Field({ label, prefix, suffix, value, onChange, min, max, sub, disabled, xs, placeholder, tip }: FieldProps) {
  ...
  const onBlur = () => { g.clearBuf(); const n = value; if (min != null && n < min) onChange(min); else if (max != null && n > max) onChange(max); };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", border: "1px solid " + (disabled ? "#e8e8e8" : C.border), borderRadius: 7, overflow: "hidden", background: disabled ? "#F4F4F4" : C.white }}>
        ...
        <input ref={g.ref} type="text" inputMode="decimal" value={g.display} disabled={!!disabled} placeholder={placeholder || "0"} onChange={g.onInput} onBlur={onBlur}
