...
export function Pill({ text, lvl }: { text: ReactNode; lvl?: Level | "n" }) {
  const m: Record<string, [string, string]> = { good: ["#DFF2EC", "#14705A"], warn: ["#FDF3E3", "#8C5A0A"], bad: ["#FCEAEC", "#9B2335"], n: ["#F7F8FA", "#4A5568"] };
...
interface Pos { left: number; top: number; W: number; arrow: number; below: boolean }
export function Info({ lines, tint }: { lines: string[]; tint?: string }) {
  ...
  const show = () => {
    ...
    try {
      const r = el.getBoundingClientRect(), vw = window.innerWidth, W = Math.min(240, vw - 16);
  ...
  useEffect(() => {
    if (!s) return;
    const h = () => setS(false);
    document.addEventListener("click", h); document.addEventListener("scroll", h, true);
    return () => { document.removeEventListener("click", h); document.removeEventListener("scroll", h, true); };
  }, [s]);
  return (
    <span style={{ position: "relative", display: "inline-block", marginLeft: 4 }}>
      <span ref={ref} role="button" tabIndex={0} aria-label="More info"
        onMouseEnter={show} onMouseLeave={hide}
        onClick={(e) => { e.stopPropagation(); s ? hide() : show(); }}
        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.stopPropagation(); s ? hide() : show(); } else if (e.key === "Escape") hide(); }}
        style={{ cursor: "pointer", color: tint || C.muted, fontSize: 13, fontWeight: 700, userSelect: "none", padding: "0 2px" }}>ⓘ</span>
      {s && pos && (
        <div role="tooltip" style={{ position: "fixed", left: pos.left, top: pos.top, transform: pos.below ? "none" : "translateY(-100%)", width: pos.W, background: "#0B1220", color: "#fff", padding: "10px 14px", borderRadius: 10, fontSize: 11, zIndex: 1200, boxShadow: "0 10px 30px rgba(0,0,0,0.6)", border: "1px solid rgba(255,255,255,0.22)", whiteSpace: "normal", pointerEvents: "none" }}>
...
export function PLRow({ label, value, neg, pos, bold, hl, indent, note }: { label: ReactNode; value: ReactNode; neg?: boolean; pos?: boolean; bold?: boolean; hl?: boolean; indent?: boolean; note?: ReactNode }) {
...
export function MBox({ label, value, sub, lvl, bar, bMax, bGood, bWarn, bInv, tip }: { label: ReactNode; value: ReactNode; sub?: ReactNode; lvl?: Level; bar?: number; bMax?: number; bGood?: number; bWarn?: number; bInv?: boolean; tip?: string[] }) {
