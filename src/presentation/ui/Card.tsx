...
interface CardProps {
  title: ReactNode; icon?: string; children?: ReactNode; right?: ReactNode;
  summary?: ReactNode; collapsible?: boolean; defaultOpen?: boolean; storeKey?: string;
}
...
export function Card({ title, icon, children, right, summary, collapsible, defaultOpen = true, storeKey }: CardProps) {
  const [open, setOpen] = useState<boolean>(() => {
    if (collapsible && storeKey) { const m = loadCardState(); if (storeKey in m) return !!m[storeKey]; }
    return defaultOpen;
  });
