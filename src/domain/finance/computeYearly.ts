...
export function computeYearly(state: Deal, R: BaseMetrics): YearlyResult {
  ...
  const appPct = (projection.appreciationPct || 0) / 100,
    price = state.price || 0;
  const exitCapEnabled = projection.exitCapEnabled,
    exitCap = (projection.exitCapRate || 6) / 100;
  let balance = R.loan,
    mr = (financing.rate ?? 7.25) / 100 / 12,
    pmt = R.pmt;
  ...
  for (let y = 1; y <= years; y++) {
    ...
    if (refiEnabled && y === refiYear && balance > 0) {
      const nr = (projection.refiRate || 6.5) / 100 / 12,
        rem = Math.max(1, origN - monthsElapsed);
      pmt = (balance * nr * Math.pow(1 + nr, rem)) / (Math.pow(1 + nr, rem) - 1);
      mr = nr;
    }
    ...
    yearly.push({
      year: y,
      monthlyRent: Math.round(mRent / R.numU),
      gpi: Math.round(gpiY),
      noi: Math.round(noiY),
      debtService: Math.round(annDebt),
      cf: Math.round(cfY),
      propVal: Math.round(propVal),
      balance: Math.round(Math.max(0, balance)),
      equity: Math.round(propVal - Math.max(0, balance)),
  ...
  for (let i = 0; i < 200; i++) {
    let npv = 0,
      d = 0;
    flows.forEach((f, j) => {
      npv += f / Math.pow(1 + irr, j);
      d -= (j * f) / Math.pow(1 + irr, j + 1);
    });
  ...
  const totCF = yearly.reduce((s, y) => s + y.cf, 0),
    deprBen = ((price * 0.85) / 27.5) * 0.28 * years;
  const appGain = (last.propVal || 0) - price,
    equityBuild = R.loan - (last.balance || 0);
