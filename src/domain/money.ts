...
export const fmt = (n: number): string => new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(n || 0);
...
export function editNumber(raw: string, sel: number, decimals: boolean): { display: string; caret: number; value: number } {
  ...
  let pos = 0,
    seen = 0;
  while (pos < display.length && seen < leftSig) {
    if (SIG.test(display[pos])) seen++;
    pos++;
  }
