...
let _uid = 10;
export function uid(): number { return ++_uid; }

let _dseq = 0;
export function newDealId(): string { return "d" + Date.now().toString(36) + (_dseq++).toString(36); }
...
export function fullState(d?: Partial<Deal> | null): Deal {
  ...
  return {
    ...
    expenses: { ...DEX, ...(migrateExpenses(d.expenses as never, units.length) || {}) },
...
export interface DealMeta { label?: string; ts?: number; created?: string | number; _name?: string }
...
export function makeDeal(data?: Partial<Deal>, meta?: DealMeta): Deal {
  ...
  void _id; void _ts; void _created;
