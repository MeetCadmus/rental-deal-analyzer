...
function field(display, caret, decimals) {
  display = display || "";
  caret = caret == null ? display.length : caret;
  return {
    display,
    caret,
    decimals: !!decimals,
    type(ch) {
      const raw = this.display.slice(0, this.caret) + ch + this.display.slice(this.caret);
      const r = M.editNumber(raw, this.caret + ch.length, this.decimals);
    ...
    back() {
      if (this.caret > 0) {
        const raw = this.display.slice(0, this.caret - 1) + this.display.slice(this.caret);
        const r = M.editNumber(raw, this.caret - 1, this.decimals);
        this.display = r.display;
        this.caret = r.caret;
        this.value = r.value;
      }
...
test("Bug 2: typing 6 . 0 yields 6.0 with caret staying after the dot", () => {
  ...
  f.type(".");
  assert.equal(f.display, "6.");
  assert.equal(f.caret, 2, "caret after the dot");
  f.type("0");
  assert.equal(f.display, "6.0");
  assert.equal(f.value, 6);
});
